...
impl RenderNode for MainPass {
    fn setup(
        ...
        // GPU buffers
        let vbuf = render_ctx.create_vertex_buffer(&vertices);
        let ibuf = render_ctx.create_index_buffer(&indices);
        let pbuf = render_ctx.create_uniform_buffer(&self.params);
        ...
        let params_layout =
            render_ctx.create_descriptor_set_layout(DescriptorSetLayoutDescriptor {
                label: Some("mainpass_params_layout"),
                visibility: StageFlags::FRAGMENT,
                layout: &[DescriptorBindingType::UniformBuffer], // binding(0)
            });

        // UBO set
        let params_set = render_ctx.build_descriptor_set(
            DescriptorSet::builder(&params_layout)
                .label("mainpass_params_set")
                .uniform(0, self.params_buffer.as_ref().unwrap()),
        );

        self.params_layout = Some(params_layout.clone());
        self.params_set = Some(params_set);
        ...
        // This pass draws straight to the surface (like your original MainPass)
        RenderNodeDescriptor {
            shader,
